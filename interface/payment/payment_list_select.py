...
class PaymentListSelect(object):
    """ 支付查询
    /api/tms/pay/listPay """

    def __init__(self):
        config = ReadYaml(FileUtil.getProjectObsPath() + '/config/config.yaml').getValue()
        self.__selectPayListApiUrl = "https://{0}:{1}{2}/api/tms/pay/listPay".format(
    ...
    def payment_list_select(self, currentPage='1', rows='10', searchMode='general', globalCondition='', sendCity='',
        ...
        try:
            ...
            response = HttpClient().get(self.__selectPayListApiUrl, self.__head_dict, payload)
            return response
        except Exception:
            return None

if __name__ == '__main__':
    test = PaymentListSelect()
    a = test.payment_list_select().json()
    print(a)
