...
class CountAmtGet(object):
    ...
    '''
    __slots__ = ('__countAmtGetApiUrl', '__head_dict')

    def __init__(self):
        config = ReadYaml(FileUtil.getProjectObsPath() + '/config/config.yaml').getValue()
        self.__countAmtGetApiUrl = 'https://{0}:{1}{2}/api/tms/pay/countAmt'.format(
    ...
    def count_amt_get(self, countName, applyDateFirst='', applyDateLast='', sendCity='', arriveCity='', carNo='',
                      driverName='', isCanLoan='',globalCondition='',searchMode='searchMode:general'):
        try:
            payload = {
            ...
            response = HttpClient().get(self.__countAmtGetApiUrl, self.__head_dict, payload)
            return response
