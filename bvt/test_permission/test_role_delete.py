...
class TestRoleDelete(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Log()
        self.logger.info('########################### TestRoleDelete START ###########################')
    ...
    def tearDown(self):
        self.logger.info('############################ TestRoleDelete END ############################')

    def test_role_delete_success(self):
        '''删除角色'''
        response = RoleDelete().role_delete(self.roleId)
        self.logger.info('删除角色返回状态码：{0}'.format(response))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['code'], 0)
        self.logger.info('删除角色返回结果是：{0}'.format(response.json()))
