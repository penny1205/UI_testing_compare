...
from interface.permission.user_passwd_update import UserPasswordUpdate
...
class TestUserPasswordUpdate(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.userId = CreateUser().create_user(roleName,menuJson,name,username,self.mobile,isLoginApp,carType)[0]
    ...
    def test_user_get_success(self):
        '''重置账号密码'''
        response = UserPasswordUpdate().user_password_update(self.userId,self.mobile)
        self.logger.info('重置账号密码返回状态码：{0}'.format(response))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['code'], 0)
        self.logger.info('重置账号密码返回结果是：{0}'.format(response.json()))
