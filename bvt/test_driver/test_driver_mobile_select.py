...
class TestDriverMobileSelect(unittest.TestCase):
    '''录单时手机号查询'''
    def setUp(self):
        self.logger = Log()
        self.logger.info('########################### TestDriverMobileSelect START ###########################')
    ...
    def tearDown(self):
        self.logger.info('############################ TestDriverMobileSelect END ############################')
        pass

    def test_driver_mobile_select_success(self):
        '''录单时按手机号查询'''
        response = DriverMobileSelect().driver_mobile_select(mobile=self.mobile)
        self.logger.info('录单时按手机号查询返回状态码：{0}'.format(response))
        ...
        if driver_list != None:
            ...
            self.assertIn(self.loginId, L, 'Driver selected fail by mobile when recording the order!')
        else:
