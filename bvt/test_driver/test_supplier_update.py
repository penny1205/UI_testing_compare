...
from interface.driver.supplier_update import SupplierUpdate
from interface.driver.supplier_get import SupplierGet
from bvt.common.create_supplier import CreateSupplier

class TestSupplierUpdate(unittest.TestCase):
    '''修改供应商信息'''
    def setUp(self):
        self.logger = Log()
    ...
    def tearDown(self):
        self.logger.info('############################ TestSupplierUpdate END ############################')

    def test_supplier_Update_name_success(self):
        '''修改供应商姓名'''
        self.name_update = '可口可乐集团'
        response = SupplierUpdate().supplier_update(self.supplierId,self.name_update, '1', '李经理', self.contactPersonMobile,
        ...
        self.logger.info('修改供应商名称返回状态码：{0}'.format(response))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['code'], 0)
        self.logger.info('修改供应商名称返回：{0}'.format(response.json()))
        response_get = SupplierGet().supplier_get(self.supplierId)
        self.assertEqual(self.name_update, response_get.json()['content']['name'], 'Supplier updated fail!')
