...
class TestSupplierGet(unittest.TestCase):
    '''获取供应商详情'''
    def setUp(self):
        self.logger = Log()
        self.logger.info('########################### TestSupplierGet START ###########################')
        self.name = '哇哈哈集团'
    ...
    def tearDown(self):
        self.logger.info('############################ TestSupplierGet END ############################')

    def test_supplier_delete_success(self):
        '''获取供应商详情'''
        response = SupplierGet().supplier_get(supplierId=self.supplierId )
        self.logger.info('获取供应商详情返回状态码：{0}'.format(response))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['code'], 0)
        self.logger.info('获取供应商详情返回：{0}'.format(response.json()))
